import org.testng.Assert;
import org.testng.annotations.*;

public class ConfigurationAnnotationsTestNg extends BaseTestNg {

    @BeforeSuite
    public void beforeSuite()
    {
        System.out.println("Chrome - Set Up System Property");
    }

    @BeforeClass
    public void beforeClass()
    {
        System.out.println("Open Test Application");
    }

    @BeforeMethod
    public void beforeMethod()
    {
        System.out.println("Sign In");
    }

    @Test
    public void searchCustomer()
    {
        System.out.println("Search For Customer");
        Assert.assertEquals("Search For Customer", "Search For Customer2", "Message");
    }

    @Test
    public void searchProduct()
    {
        System.out.println("Search For Product");
        if (true) {
            Assert.fail("If condition true");
        }
    }

    @AfterMethod
    public void afterMethod()
    {
        System.out.println("Sign Out");
    }

    @AfterClass
    public void afterClass()
    {
        System.out.println("Close Test Application");
    }


    @AfterSuite
    public void afterSuite()
    {
        System.out.println("Chrome - Clean Up All Cookies");
    }

}
